...
module.exports = function(homebridge){
    ...
    homebridge.registerAccessory('homebridge-lightbar', 'HTTP-RGB', HTTP_RGB);
};
...
function HTTP_RGB(log, config) {
 ...
 //Mode Handling
    if (typeof config.mode === 'object') {
        this.mode = {};
        this.mode.status         = config.mode.status;
        this.mode.set_url        = config.mode.url            || this.mode.status;
        this.mode.http_method    = config.mode.http_method    || this.http_method;
        this.cache.mode = 0;
    } else {
        this.brightness = false;
        this.cache.brightness = 100;
...
HTTP_RGB.prototype = {
    ...
    getServices: function() {
        ...
        switch (this.service) {
            case 'Light':
                ...
                //Handle Mode
                if (this.has.mode) {
                    this.log('... adding mode');
                    lightbulbService
                        .addCharacteristic(new Characteristic.mode())
    ...
    getmode: function(callback) {
        if (!this.has.mode) {
            this.log.warn("Ignoring request; No 'mode' defined.");
            callback(new Error("No 'mode' defined in configuration"));
            return;
        }

        if (this.mode) {
            this._httpRequest(this.mode.status, '', 'GET', function(error, response, responseBody) {
                if (error) {
                    this.log('getmode() failed: %s', error.message);
                    callback(error);
                } else {
                    var level = parseInt(responseBody);
                    this.log('mode is currently %s ', level);
                    callback(null, level);
                }
            }.bind(this));
        } else {
            callback(null, this.cache.mode);
        }
    },
    /**
    ...
    setmode: function(level, callback) {
        if (!this.has.mode) {
            this.log.warn("Ignoring request; No 'mode' defined.");
            callback(new Error("No 'mode' defined in configuration"));
            return;
        }
        this.cache.mode = level;
   
            var url = this.mode.set_url.replace('%s', level);

            this._httpRequest(url, '', this.mode.http_method, function(error, response, body) {
                if (error) {
                    this.log('setmode() failed: %s', error);
                    callback(error);
                } else {
                    this.log('setmode() successfully set to %s ', level);
                    callback();
